...
- 求人の場合:
  - 時給・給料の良さを目立たせる
  - 働きやすさをアピール（「週1日からOK」「未経験でも大丈夫」など）
  - 勤務地のアクセスの良さ（駅チカなど）
  - 外国人が安心できるポイント（日本語レベル、チームワーク、研修あり等）

### テキストメッセージの書き方
- 親しみやすい挨拶 + この仕事の一番の魅力を短くキャッチーに伝える
- **改行を積極的に使って読みやすくする**
- 情報を詰め込まず、1行ずつ区切って読みやすいリズムにする
- 例:
  こんにちは！😊

  渋谷で時給1,500円のお仕事が出ました！
...
### カードメッセージの書き方
- タイトル: 職種名
- body: **以下の4項目のみ、シンプルに記載**（詳細は求人ページに遷移させるため）
  💰 時給・給料
  📍 勤務地
  🗣️ 日本語レベル
  🕐 勤務時間
- 「応募する」ボタン付き
- **それ以外の情報（待遇、応募条件、仕事内容等）はカードに書かない**

## 絵文字ルール（ブランド統一のため必ず守ること）
以下の絵文字を固定で使ってください。毎回同じ絵文字を使うことでブランドイメージを統一します。
- 💰 給料・時給
- 📍 勤務地・場所
- 🕐 勤務時間・シフト
- 🗣️ 日本語レベル
上記以外の絵文字は自由に使ってOKですが、上記の項目には必ず指定の絵文字を使ってください。
...
- **改行を積極的に使って読みやすくする**（情報を詰め込まず、1行ずつ区切る）

### カードメッセージの場合
- body: **以下の4項目のみ記載**（詳細は求人ページで確認させるため）
  💰 時給・給料
  📍 勤務地
  🗣️ 日本語レベル
  🕐 勤務時間
...
${formatInstruction}`;
    ...
    const CONTENT_TYPE_HINTS: Record<string, string> = {
      ...
      contact: '\n## 配信の種類: お問い合わせ案内\nお問い合わせ方法や窓口を分かりやすく案内するメッセージを作成してください。問い合わせ先・受付内容・対応時間を明確にし、気軽に問い合わせできる雰囲気にしてください。',
    };
